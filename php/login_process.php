<?php
    ...
    if($valid)
    {
        ...
        if($statement = $connect->prepare("SELECT username,password FROM customer WHERE username LIKE ? AND password LIKE ?")){
            ...
            if(!$found){
                //check staff table
                $statement = $connect->prepare("SELECT s.username,s.password,p.position_name FROM staff s,position p WHERE username LIKE ? AND password LIKE ? AND s.position_id = p.position_id");
                $statement->bind_param("ss",$username,$hashed);
                $statement->execute();
                $result = $statement->get_result();
                if($result->num_rows>0){
